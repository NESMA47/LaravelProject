...
| `admin` | Full platform control: manage users, moderate jobs & reviews, manage categories & skills, view audit logs, resolve reports |
...
4. **AuditLog** — Fire-and-forget write to `audit_logs` for `DELETE`, `PATCH status`, `POST` on sensitive resources.
